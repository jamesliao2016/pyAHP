...
df = sql('''select * from app.app_pa_features_dtcid3  where dt = '2018-01-20' ''')
df = df.drop('dt')
df.cache()
...
# fi_pd = spark.read.csv("hdfs:///user/mart_rmb/liaopeng/app_pa_festival_information_0529",encoding='UTF-8')

fi_df=sql("""select * from app.app_pa_festival_information """)
