...
if len(sys.argv) >= 5:
    params['shop_id'] = sys.argv[4]
# 销量基线update_end以前的所有历史数据
params['shop_id'] = 598847
shop_id = params['shop_id']
# update_start = params['update_origin']
# update_end = params['update_end']
update_start = '2017-06-01'
update_end = '2018-06-01'
...
FROM %s
WHERE
    dt <= '%s' and item_sku_id in (758350,766923,889167,976141,1084804)
...
result_df = result_df.withColumn('shop_id',F.lit(shop_id))
spark.sql("set hive.exec.dynamic.partition=true")
spark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
logger.info('Saving results...')
logger.info('inserting app.app_p101_baseline_sku_0608_3...')
...
save_result(result_df,
                      'app.app_p101_baseline_sku_0608_7',
                      partitioning_columns=['shop_id','dt'], repartitioning_columns=['date'],
